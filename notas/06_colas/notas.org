...
- *front* es acceder al primer elemento de la cola
...
La cola se va a ver de la siguiente manera:
|    1 |<- head
|    2 |
|    3 |<- cola


Asi que al ejecutar dequeue() o front() nos va a dar 1.

Al igual que las listas, cada cola se compone de nodos los cuales encapsula:
- value (valor del tipo)
- next (referencia al siguiente nodo)

* Implementacion
La complejidad de la cola es como implementar la insercion y eliminacion o enqueue y dequeue, la busqueda y el recorrido y el preguntar si es vacia es el mismo que venimos trabajando.
...
** Dequeue
De igual manera, hay que cuidarnos que no queramos desencolar de una cola vacia, asi que por diseño hay que poner esas validaciones, ya que vamos a romper el programa, en caso de que si sea vacia la cola se regresa una excepcion.
...
En lugar de tomar en cuenta quien llego primero, toma en cuenta prioridades dependiendo del valor del elemento, su ejemplo mas comun es un monticulo o heap, donde cada vez que hacemos dequeue queremos que salga el elemento mas pequeño, no importa si acaba de ser ingresado o fue el primero, queremos que siempre salga el mas pequeño.
* Operaciones
...
De igual manera no podemos hacer dequeue sin haber revisado si la cola es vacia
...
Agregar scheduling, BFS, buffers E/S
